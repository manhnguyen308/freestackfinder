...
| Free plan star ratings | Day 87e | Trust | `layouts/shortcodes/rating.html` renders 1 to 5 teal stars in half steps under each tool heading, with a "How we rate" link to the rubric on `/about/#star-ratings`. 238 ratings across all 50 articles; every score and its reason is in `docs/RATINGS.md`. Editorial scores from published free limits, not user reviews; no Review or Rating JSON-LD because Google's review snippets do not cover ratings of items in a list. `publish_checklist.py` fails an unrated tool section |
| Light/dark theme toggle | Day 87f | UX | Header icon button after search switches themes; first visit follows the system setting, a click saves the choice in `localStorage`. An inline script in `head.html` sets `html[data-theme]` before paint, so there is no light flash. Dark token values live in `style.css` section 35 inside `@media screen`, so print stays light. New `--primary-fill` tokens keep white-on-teal fills at 5.5:1 in dark mode; the light theme renders as before. Hidden without JavaScript |
| Homepage hero search | Day 87h | Discovery | `layouts/index.html` hero has a GET form to `/search/` with a `q` field, above the "Browse all guides" button. Enter or the Search button opens `/search/?q=<keyword>`, which `search.js` already reads to prefill the box and run the search. Works without JavaScript on the homepage; styles use tokens only, so both themes need no override |
| Header search panel | Day 87j | Discovery | The nav search icon opens a panel under the header (`layouts/partials/nav.html`, `static/js/main.js`) with a GET form to `/search/?q=`. Closes on Escape, its close button, an outside click, or the mobile menu; on `/search/` the icon focuses the page box instead. Without JavaScript the icon stays a link to `/search/`. Shares the `.search-form` styles with the hero |
